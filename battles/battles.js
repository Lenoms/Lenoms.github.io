...
function get_puzzle_answers(puzzle_number){
	if (puzzle_number == '1'){return ['water (K)','Water (K)', 'water (k)', 'Water (k)','Water(K)','water(K)']}
	else if (puzzle_number == '2'){return ['what','What','stick','Stick']}
...
// Reveals next connection song in the series
...
// Local answer check, since multiple things would be acceptable.
function check_local_answer(level_num, answer){
	var allowable_answers = "^(titletracks|title tracks|titlesongs|title songs|albumnametracks|album name tracks|albumnamesongs|album name songs|songs with same name as album|songswithsamenameasalbum|tracks with same name as album|trackswithsamenameasalbum|songs that match album name|tracks that match album name)$";
